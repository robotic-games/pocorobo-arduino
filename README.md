...
3. **スタジオ用のプログラム（標準のプログラム）が入っている本体は、最初に Arduino 用に切り替える操作が必要です**（標準のプログラムが動いている間は、USB がパソコンとのネットワーク接続として動いていて、書き込み口が見えないため）。切り替えの手順は準備中です。一度 Arduino のスケッチが入れば、以後はケーブルをつなぐだけで書き込めます
4. 「ツール → ポート」で本体のポートを選び、［→］（書き込み）を押します。書き込みが終わると自動で再起動して動き出します

2 回目からはボタン操作なしで書き込めます。「ツール → シリアルモニタ」（115200）で `Serial.println()` の出力が見えます。
...
| 症状 | 対処 |
|---|---|
| ポートが出てこない | 標準のプログラムが動いている本体は、先に Arduino 用への切り替えが必要です（準備中）。ケーブルがデータ通信に対応しているかも確認してください |
